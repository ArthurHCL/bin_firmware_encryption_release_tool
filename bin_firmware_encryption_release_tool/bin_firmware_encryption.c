...
int bin_firmware_encryption(void)
{
	...
	original_firmware_bin_file = fopen(original_firmware_bin_file_name, "rb");
	...
	released_firmware_bin_file = fopen(released_firmware_bin_file_name, "wb");
	...
	*/
	...
	while (!is_original_firmware_bin_file_read_complete) {
		...
		if (read_items_amount) {
			...
			*/
			if (AES_BLOCKLEN > read_items_amount) {
				for (i = (int)read_items_amount; i < AES_BLOCKLEN; i++) {
					data_buf[i] = 0xFF;
				}
			}
