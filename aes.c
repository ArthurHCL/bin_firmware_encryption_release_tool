/*
    ...
    specifically ECB, CTR and CBC mode.
        block size can be chosen in aes.h,
    available choices are AES128, AES192, AES256.
...
/*
    ...
    contains the values given by x to the power (i-1) being powers of x (x is denoted as {02}) in the field GF(2^8).
*/
...
*/
static void KeyExpansion(uint8_t * const RoundKey, const uint8_t * const Key)
{
    unsigned int i, j, k;
...
/*
        add the round key to state.
    the round key is added to the state by an XOR function.
*/
static void AddRoundKey(const uint8_t round, state_t * const state, const uint8_t * const RoundKey)
{
    unsigned int i, j;

    for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j) {
            (*state)[i][j] ^= RoundKey[(4 * Nb * round) + (Nb * i) + j];
        }
    }
}

/* substitutes the values in the state matrix with values in an S-box. */
static void SubBytes(state_t * const state)
{
    int i, j;
...
*/
static void ShiftRows(state_t * const state)
{
    uint8_t temp;
...
static void MixColumns(state_t * const state)
{
    int     i;
    uint8_t Tmp, Tm, t;
...
*/
static void InvMixColumns(state_t * const state)
{
    int     i;
    uint8_t a, b, c, d;
...
/* substitutes the values in the state matrix with values in an S-box. */
static void InvSubBytes(state_t *const state)
{
    int i, j;
...
static void InvShiftRows(state_t * const state)
{
    uint8_t temp;
...
static void Cipher(state_t * const state, const uint8_t * const RoundKey)
{
    uint8_t round;
...
static void InvCipher(state_t * const state, const uint8_t * const RoundKey)
{
    uint8_t round;
